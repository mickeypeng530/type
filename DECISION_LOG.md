...
## 2026-08-14 展開器:結尾字元可能是更長縮寫的一部分,要先探一步
- 症狀:打 `bd/`(想要 bulging disc.)在按下 `/` 的瞬間就被展成 `bile duct`,結果是 `bile duct/`。
- root cause:`/` 既在 AHK 預設結尾字元集合裡,又是 **47 組縮寫**的結尾字元
  (`bd`/`bd/`、`dil`/`dil/`、`gbs`/`gbs/`、`lsbd`/`lsbd/`…)。原本一看到結尾字元就展開最長匹配,
  但當下的最長匹配是短的那個。
- 修法:`expandBefore(ta, endChar)` —— 先把待輸入的結尾字元接上去再跑一次 `expMatch`,
  若能匹配到**更長**的縮寫就先不展開,等下一個結尾字元。
- 沒選:①把 `/` 從結尾字元集合拿掉(那 194 條以 `/` 結尾的縮寫就得多打一個字元才會展開,與 AHK 不一致)
  ②只對「已知有長版」的縮寫做特例(要維護一張表,新增縮寫時會忘)。
- 狀態:active

## 2026-08-21 計數列顯示當日金額:單價存 RTDB,不寫死也不外送
- 決策:計數列最右顯示當日金額;單價六項存 `voiceReport/counter/prices`(借住 counter 子樹,共用帳號可寫),
  在計數面板內就地編輯。**金額不進任何給 Worknum 的匯出**。
- 依據:`中興log_交換格式.md` §1 明講「不要傳金額,單價是 Worknum settings 的責任」,
  但同一句也寫「你那邊要顯示金額自己算沒關係」→ 顯示可以,傳輸不行。
- 沒選:①把單價寫死在程式碼(費率一變就要改 code 重部署,而且兩邊硬編必定漂移)
  ②從 Worknum 的 `settings` 讀(查過:那邊只有 `unitPrices`(本院),**沒有** `xhPrices`,
  中興費率不在 DB 裡)③缺單價時用已知項目先加總(會給出一個看起來正常、實際少算的金額 —— 最危險)。
- 過程紀錄:一開始只有「2026-08-07 共 24 件 = $16,800」一個資料點,推出「一律 700/件」剛好吻合;
  後來使用者貼出 Worknum 面板才知道實際是六種不同單價(1000/1000/800/200/700/450),
  同一天用真實單價回推也是 16,800 —— **單一資料點無法辨別兩種假設**。錢的事沒拿到來源就別填。
- 狀態:active
