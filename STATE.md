...
- **計數器可輸出給 Worknum**:面板下方有「在中興」起訖時間欄 + 「📋 複製給 Worknum」,
  ...
  「到 / 離」時間欄與「🧮 計數 / 日期」同一排;值存當日物件的 `start`/`end`,不參與件數統計。
  ⚠️ **手填、不由逐筆推導**(交換格式 §3.6):到達時間 ≠ 第一筆報告的時間,中間有啟程/準備/找片子,
  自動用首末筆去填會把準備時間吃掉、讓 $/hr 高估。判讀時段那組由 Worknum 自己從首末筆算,兩組互不覆蓋。
...
- **逐筆時間戳(2026-08-13)**:每次 ±1 / 改數字 / 打勾都記一筆 `{t:"HH:MM", k:項目, d:增減}`
  ...
  匯出**一顆鈕**「📋 給 Worknum」,依當天有沒有逐筆**二選一**(交換格式 §3.5:送逐筆時
  不要再送彙總列,單一事實來源):有逐筆 → `arrive` + 逐筆 N 列 + `leave`;沒有 → 9 欄彙總列。
  欄數辨識:9 = 彙總 / 4 = 逐筆 / 3 = arrive|leave。
